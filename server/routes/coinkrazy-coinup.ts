...
// Symbol types
type SymbolType = 'coin-01' | 'coin-05' | 'coin-10' | 'coin-25' | 'coin-50' | 'coin-100' | 'multiup' | 'coinup' | 'bonus' | 'jackpot';
...
const SYMBOL_PAYOUTS: Record<SymbolType, number[]> = {
  'coin-01': [0.1, 0.2, 0.3, 0.5, 1],
  'coin-05': [0.2, 0.4, 0.6, 1, 2],
  'coin-10': [0.5, 1, 1.5, 2, 3],
  'coin-25': [1, 2, 3, 4, 5],
  'coin-50': [1.5, 3, 4.5, 6, 8],
  'coin-100': [2, 4, 6, 8, 10],
  'multiup': [0, 0, 0, 0, 0], // Multiplier symbol
  'coinup': [0, 0, 0, 0, 0], // Bonus symbol
  'bonus': [0.5, 1, 2, 4, 8],
...
function generateSymbol(): SymbolType {
  const symbols: SymbolType[] = [
    'coin-01', 'coin-05', 'coin-10', 'coin-25', 'coin-50', 'coin-100', 'multiup', 'bonus'
  ];
  ...
  // Random chance for coinup bonus
  if (Math.random() < 0.05) {
    return 'coinup';
  }
...
function calculateWin(reels: SymbolType[][], bet: number): { win: number; description: string } {
  ...
  for (let payline = 0; payline < 15; payline++) {
    ...
    if (baseSymbol === 'coinup') {
      // Coinup symbol triggers bonus collection
      winners.push('coinup-trigger');
      totalWin += bet * 2; // Base coinup reward
      continue;
    }
    ...
    for (let i = 1; i < 5; i++) {
      ...
      if (current === baseSymbol || current === 'multiup' || baseSymbol === 'multiup') {
        matchCount++;
      } else {
        break;
      }
    ...
    if (matchCount >= 3 && baseSymbol !== 'multiup' && baseSymbol !== 'coinup') {
...
// Main spin handler
export const handleCoinKrazyCoinUpSpin: RequestHandler = async (req, res) => {
  try {
    ...
    await query(
      `INSERT INTO game_results (game_id, player_id, bet_amount, win_amount, multiplier, result_data, status, played_at)
       VALUES (
         (SELECT id FROM games WHERE name = 'CoinKrazy-CoinUp' LIMIT 1),
    ...
    console.error('[CoinKrazy-CoinUp] Spin error:', error);
    res.status(500).json({
